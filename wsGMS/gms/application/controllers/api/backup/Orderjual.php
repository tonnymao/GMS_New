...
class Orderjual extends REST_Controller { 
    ...
    function insertorderjual_post(){     
        ...
        $datalistbarang        = (isset($jsonObject["datalistbarang"])        ? $jsonObject["datalistbarang"]        : "");
        $datalistbiayaestimasi = (isset($jsonObject["datalistbiayaestimasi"]) ? $jsonObject["datalistbiayaestimasi"] : "");
        $datalistbiayainternal = (isset($jsonObject["datalistbiayainternal"]) ? $jsonObject["datalistbiayainternal"] : "");

        $bex_nomor = $this->db->query("SELECT intNomorMSales FROM muser_android where intNomorMUser ='". $sales."'")->row()->intNomorMSales; 
        ...
        $newDate = date("ym", strtotime($tanggal));

        $querykode = "  SELECT CONCAT('SO/$newDate/',LPAD(IFNULL((MAX(SUBSTRING(vcKode,9)) + 1),1),5,'0')) AS vcKodeBaru
                        FROM thorderjual 
                        WHERE intStatus = 1 AND SUBSTRING(vcKode,1,8) = 'SO/$newDate/'";
        ...
        if($datalistbarang != ""){
            $pieces = explode("|", $datalistbarang);
            foreach ($pieces as $arr) {
                $valuedata = explode("~", $arr);

                if( $valuedata[0] != ""){
                    $query_detail_barang = $this->db->insert_string('tdorderjual', array(
                                                                          'intNomorHeader'      =>$header_nomor, 
                                                                          'intNomorMBarang'     =>$valuedata[0], 
                                                                          'intNomorMSatuan'     =>$valuedata[2], 
                                                                          'intNomorMSatuanUnit' =>$valuedata[3], 
                                                                          'decJumlah'           =>$valuedata[4], 
                                                                          'decHarga'            =>$valuedata[6], 
                                                                          'decJumlahUnit'       =>$valuedata[7], 
                                                                          'decDisc1'            =>$valuedata[9], 
                                                                          'decDisc2'            =>$valuedata[10], 
                                                                          'decDisc3'            =>$valuedata[11], 
                                                                          'decNetto'            =>$valuedata[12], 
                                                                          'decSubTotal'         =>$valuedata[13]
                                                                        )
                                                    );
                    $this->db->query($query_detail_barang);
                    // array_push($data['data'], array( 'success' => $query_detail_barang ));

                    $detail_nomor = $this->db->insert_id();
                    $query_detail_shade = $this->db->insert_string('tdorderjualshade', array(
                                                                          'intNomorHeader'      =>$header_nomor, 
                                                                          'intNomorDetail'      =>$detail_nomor, 
                                                                          'intNomorMBarang'     =>$valuedata[0], 
                                                                          'vcShade'             =>$valuedata[14], 
                                                                          'decJumlah'           =>$valuedata[15]
                                                                        )
                                                    );
                    $this->db->query($query_detail_shade);
                }
            }
        }

        if($datalistbiayaestimasi != ""){
            $pieces = explode("|", $datalistbiayaestimasi);
            foreach ($pieces as $arr) {
                $valuedata = explode("~", $arr);

                if( $valuedata[0] != ""){
                    $ppnnominal_data = floatval($valuedata[8]) * floatval($valuedata[7]) / 100;
                    $query_detail_estimasi = $this->db->insert_string('tdorderjualbiaya', array(
                                                                          'intNomorHeader'      =>$header_nomor, 
                                                                          'intNomorMJenisBiaya' =>$valuedata[1], 
                                                                          'intNomorMSupplier'   =>$valuedata[2], 
                                                                          'vcDeskripsi'         =>$valuedata[4], 
                                                                          'intNomorMValuta'     =>"1", 
                                                                          'decKurs'             =>"1", 
                                                                          'decTotalSupplier'    =>$valuedata[5], 
                                                                          'decPPN'              =>$valuedata[7], 
                                                                          'decPPNNominal'       =>$ppnnominal_data, 
                                                                          'decSubTotal'         =>$valuedata[8], 
                                                                          'decTotal'            =>$valuedata[8], 
                                                                          'decTotalIDR'         =>$valuedata[8]
                                                                        )
                                                    );
                    $this->db->query($query_detail_estimasi);
                    // array_push($data['data'], array( 'success' => $query_detail_estimasi ));
                }
            }
        }
        ...
        if($datalistbiayainternal != ""){
            $pieces = explode("|", $datalistbiayainternal);
            foreach ($pieces as $arr) {
                $valuedata = explode("~", $arr);

                if( $valuedata[0] != ""){
                    $ppnnominal_data = floatval($valuedata[5]) * floatval($valuedata[4]) / 100;
                    $query_detail_internal = $this->db->insert_string('tdorderjualbiayainternal', array(
                                                                          'intNomorHeader'      =>$header_nomor, 
                                                                          'intNomorMJenisBiaya' =>$valuedata[1], 
                                                                          'vcDeskripsi'         =>$valuedata[2], 
                                                                          'intNomorMValuta'     =>"1", 
                                                                          'decKurs'             =>"1", 
                                                                          'decPPN'              =>$valuedata[4], 
                                                                          'decPPNNominal'       =>$ppnnominal_data, 
                                                                          'decSubTotal'         =>$valuedata[5], 
                                                                          'decTotal'            =>$valuedata[5], 
                                                                          'decTotalIDR'         =>$valuedata[5]
                                                                        )
                                                    );
                    $this->db->query($query_detail_internal);
                    // array_push($data['data'], array( 'success' => $query_detail_internal ));
                }
            }
        }
        ...
        if ($this->db->trans_status() === FALSE){
            ...
            if( $result_adminsales && $result_adminsales->num_rows() > 0){
                ...
                $sales_nama = $this->db->query("SELECT vcNama FROM muser where intNomor ='". $sales."'")->row()->vcNama; 
                $this->send_gcm_group($regisID, $this->ellipsis("Penambahan Order Jual baru"),'New Order Jual From : ' . $sales_nama,'NewCustomerList',$header_nomor,$sales_nama);
            } 
        }  
