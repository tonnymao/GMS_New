...
class Target extends REST_Controller { 
    ...
    function createTarget_post(){     
        ...
        $user_nomor  = (isset($jsonObject["user_nomor"])  ? $this->clean($jsonObject["user_nomor"])  : "15");
        $bulan  = (isset($jsonObject["bulan"])  ? $this->clean($jsonObject["bulan"])  : "8");
        $tahun  = (isset($jsonObject["tahun"])  ? $this->clean($jsonObject["tahun"])  : "2018");
        $datalisttarget = (isset($jsonObject["datalisttarget"]) ? $jsonObject["datalisttarget"] : "38~oki~123|35~anton~123");
                ...
                if( $valuedata[0] != ""){
                    ...
                    if($bex_nomor == $valuedata[0]){
                        ...
                        if($vcGCMId != null){
                            ...
                            $this->send_gcm_group($vcGCMId, $this->ellipsis("Penambahan data target baru"),'New Target From : ' . $user_nama,'Dashboard',$header_nomor,$user_nama);   
                        }     
        ...
        if ($this->db->trans_status() === FALSE){
            ...
            for($i = 0; $i < count($regisID); $i++){
                ...
                if($regisID[$i] != null){
                    ...
                    $this->send_gcm_group($regisID[$i], $this->ellipsis("Penambahan data target baru"),'New Target From : ' . $user_nama,'Dashboard',$regisID_Header[$i],$user_nama);   
                }
